...
trait MessageContainerBase {
  ...
  def Deserialize(dis: DataInputStream, mdResolver: MdBaseResolveInfo, savedDataVersion: String): Unit
...
object SerializeDeserialize {
  ...
  def Deserialize(bytearray: Array[Byte], mdResolver: MdBaseResolveInfo): MessageContainerBase = {
    ...
    try {
      ...
      val typ = mdResolver.getMessgeOrContainerInstance(typName)
      if (typ == null) {
        throw new Exception("Message/Container %s not found to deserialize".format(typName))
      }
      typ.Deserialize(dis, mdResolver, version)
      dis.close
      return typ
    } catch {
      case e: Exception => {
