...
object ClusterCfgUtils {
  ...
  def AddNode(nodeId:String,nodePort:Int,nodeIpAddr:String,
    ...
    try{
      ...
      DAOUtils.SaveObject(key.toLowerCase,value,MetadataAPIImpl.configStore)
  ...
  def RemoveNode(nodeId:String) : String = {
    try{
      ...
      val key = "NodeInfo." + nodeId
      DAOUtils.DeleteObject(key.toLowerCase,MetadataAPIImpl.configStore)
  ...
  def RemoveConfig(cfgStr: String): String = {
	...
	adapters.foreach(a => {
      ...
      DAOUtils.RemoveObjectList(keyList,MetadataAPIImpl.configStore)
  ...
  def UploadConfig(cfgStr: String): String = {
	...
	DAOUtils.SaveObjectList(keyList,valueList,MetadataAPIImpl.configStore)
  ...
  def LoadAllConfigObjectsIntoCache: Boolean = {
	...
	//logger.debug("key => " + KeyAsStr(key))
        val obj = DAOUtils.GetObject(key, MetadataAPIImpl.configStore)
