...
object MetadataAPIMessageUtils {
  ...
  def AddMessageDef(msgDef: MessageDef): String = {
    try {
      MetadataAPIUtils.AddObjectToCache(msgDef, MdMgr.GetMdMgr)
      MetadataAPIDAOUtils.UploadJarsToDB(msgDef)
      var objectsAdded = AddMessageTypes(msgDef, MdMgr.GetMdMgr)
      objectsAdded = objectsAdded :+ msgDef
      MetadataAPIDAOUtils.SaveObjectList(objectsAdded, MetadataAPIImpl.metadataStore)
      val operations = for (op <- objectsAdded) yield "Add"
      MetadataAPIUtils.NotifyEngine(objectsAdded, operations)
      //check again
  ...
  def AddMessageTypes(msgDef: BaseElemDef, mdMgr: MdMgr): Array[BaseElemDef] = {
    ...
    try {
      ...
      val msgType = MetadataAPIUtils.getObjectType(msgDef)
      ...
      msgType match {
        case "MessageDef" | "ContainerDef" => {
          ...
          MetadataAPIUtils.AddObjectToCache(obj, mdMgr)
          types = types :+ obj
  ...
  def AddContainerOrMessage(contOrMsgText: String, format: String, recompile:Boolean = false): String = {
    ...
    try {
     ...
     // compProxy.setLoggerLevel(Level.TRACE)
	  ...
	  if( recompile ){
	    val depModels = MetadataAPIModelUtils.GetDependentModels(msg.NameSpace,msg.Name,msg.ver)
	    if( depModels.length > 0 ){
	      depModels.foreach(mod => {
		logger.debug("DependentModel => " + mod.FullNameWithVer)
		resultStr = resultStr + MetadataAPIModelUtils.RecompileModel(mod)
	      })
	    }
	  }
	  resultStr
	  ...
	  if( recompile ){
            MetadataAPIContainerUtils.RemoveContainer(cont.nameSpace, cont.name, cont.ver)
            resultStr = MetadataAPIContainerUtils.AddContainerDef(cont)
	  }
	  else{
            resultStr = MetadataAPIContainerUtils.AddContainerDef(cont)
	  }
	  ...
	  if( recompile ){
	    val depModels = MetadataAPIModelUtils.GetDependentModels(cont.NameSpace,cont.Name,cont.ver)
	    if( depModels.length > 0 ){
	      depModels.foreach(mod => {
		logger.debug("DependentModel => " + mod.FullNameWithVer)
		resultStr = resultStr + MetadataAPIModelUtils.RecompileModel(mod)
      ...
      msgDef match {
        case msg: MessageDef => {
          ...
          if (latestVersion != None) {
            isValid = MetadataAPIUtils.IsValidVersion(latestVersion.get, msg)
          }
          if (isValid) {
	    ...
	    val depModels = MetadataAPIModelUtils.GetDependentModels(msg.NameSpace,msg.Name,msg.Version.toInt)
	    if( depModels.length > 0 ){
	      depModels.foreach(mod => {
		logger.debug("DependentModel => " + mod.FullNameWithVer)
		resultStr = resultStr + MetadataAPIModelUtils.RecompileModel(mod)
        ...
        case msg: ContainerDef => {
          val latestVersion = MetadataAPIContainerUtils.GetLatestContainer(msg)
          var isValid = true
          if (latestVersion != None) {
            isValid = MetadataAPIUtils.IsValidVersion(latestVersion.get, msg)
          }
          if (isValid) {
            MetadataAPIContainerUtils.RemoveContainer(latestVersion.get.nameSpace, latestVersion.get.name, latestVersion.get.ver)
            resultStr = MetadataAPIContainerUtils.AddContainerDef(msg)
    ...
    try {
      ...
      o match {
        ...
        case Some(m) =>
          ...
          var objectsToBeRemoved = MetadataAPIUtils.GetAdditionalTypesAdded(msgDef, MdMgr.GetMdMgr)
          // Also remove a type with same name as messageDef
          var typeName = name
          var typeDef = MetadataAPITypeUtils.GetType(nameSpace, typeName, version.toString, "JSON")
          if (typeDef != None) {
            objectsToBeRemoved = objectsToBeRemoved :+ typeDef.get
          }
          objectsToBeRemoved.foreach(typ => {
            MetadataAPITypeUtils.RemoveType(typ.nameSpace, typ.name, typ.ver)
          })
          // MessageDef itself
          MetadataAPIDAOUtils.DeleteObject(msgDef)
          objectsToBeRemoved = objectsToBeRemoved :+ msgDef
	  if( zkNotify ){
            val operations = for (op <- objectsToBeRemoved) yield "Remove"
            MetadataAPIUtils.NotifyEngine(objectsToBeRemoved, operations)
	  }
  ...
  def LoadMessageIntoCache(key: String) {
    try {
      ...
      val obj = MetadataAPIDAOUtils.GetObject(key.toLowerCase, MetadataAPIImpl.messageStore)
      ...
      MetadataAPIDAOUtils.DownloadJarFromDB(msgDef)
  ...
  def LoadAllMessagesIntoCache {
    try {
      val msgKeys = MetadataAPIUtils.GetAllKeys("MessageDef")
      if (msgKeys.length == 0) {
      ...
      msgKeys.foreach(key => {
        val obj = MetadataAPIDAOUtils.GetObject(key.toLowerCase, MetadataAPIImpl.messageStore)
        val msg = serializer.DeserializeObjectFromByteArray(obj.Value.toArray[Byte])
        MetadataAPIUtils.AddObjectToCache(msg.asInstanceOf[MessageDef], MdMgr.GetMdMgr)
      })
    } catch {
      case e: Exception => {
        e.printStackTrace()
