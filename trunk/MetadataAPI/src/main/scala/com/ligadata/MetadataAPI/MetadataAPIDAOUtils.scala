...
object MetadataAPIDAOUtils {
  ...
  def SaveObjectList(objList: Array[BaseElemDef], store: DataStore) {
    ...
    try {
      ...
      objList.foreach(obj => {
        obj.tranId = tranId
        val key = (MetadataAPIUtils.getObjectType(obj) + "." + obj.FullNameWithVer).toLowerCase
  ...
  def UploadJarsToDB(obj: BaseElemDef) {
    try {
      ...
      var value = MetadataAPIUtils.GetJarAsArrayOfBytes(jarName)
      ...
      if (obj.DependencyJarNames != null) {
        obj.DependencyJarNames.foreach(j => {
          ...
          if (j.endsWith(".jar")) {
            ...
            value = MetadataAPIUtils.GetJarAsArrayOfBytes(jarName)
  ...
  def DownloadJarFromDB(obj: BaseElemDef) {
    try {
      //val key:String = (MetadataAPIUtils.getObjectType(obj) + "." + obj.FullNameWithVer).toLowerCase
      if (obj.jarName == null) {
        logger.debug("The object " + obj.FullNameWithVer + " has no jar associated with it. Nothing to download..")
        return
      }
      var allJars = MetadataAPIUtils.GetDependantJars(obj)
      ...
      if (allJars.length > 0) {
        ...
        allJars.foreach(jar => {
          ...
          val b = MetadataAPIUtils.IsDownloadNeeded(jar, obj)
          if (b == true) {
            val key = jar
            val mObj = GetObject(key, MetadataAPIImpl.jarStore)
            val ba = mObj.Value.toArray[Byte]
            val jarName = dirPath + "/" + jar
            MetadataAPIUtils.PutArrayOfBytesToJar(ba, jarName)
          }
        })
      }
    } catch {
  ...
  def ModifyObject(obj: BaseElemDef, operation: String) {
    try {
      val o1 = MetadataAPIUtils.UpdateObjectInCache(obj, operation, MdMgr.GetMdMgr)
      UpdateObjectInDB(o1)
    } catch {
  ...
  def LoadAllConfigObjectsIntoCache: Boolean = {
	...
	//logger.debug("key => " + KeyAsStr(key))
        val obj = MetadataAPIDAOUtils.GetObject(key, MetadataAPIImpl.configStore)
