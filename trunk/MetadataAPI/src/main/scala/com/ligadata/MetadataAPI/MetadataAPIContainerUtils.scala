...
object MetadataAPIContainerUtils {
  ...
  def AddContainerDef(contDef: ContainerDef): String = {
    ...
    try {
      MetadataAPIUtils.AddObjectToCache(contDef, MdMgr.GetMdMgr)
      MetadataAPIDAOUtils.UploadJarsToDB(contDef)
      var objectsAdded = MetadataAPIMessageUtils.AddMessageTypes(contDef, MdMgr.GetMdMgr)
  ...
  def AddContainer(containerText: String, format: String): String = {
    MetadataAPIMessageUtils.AddContainerOrMessage(containerText, format)
  }
  ...
  def UpdateContainer(messageText: String): String = {
    MetadataAPIMessageUtils.UpdateMessage(messageText,"JSON")
  }
    ...
    try {
      ...
      o match {
        ...
        case Some(m) =>
          ...
          var objectsToBeRemoved = MetadataAPIUtils.GetAdditionalTypesAdded(contDef, MdMgr.GetMdMgr)
          ...
          // ContainerDef itself
          MetadataAPIDAOUtils.DeleteObject(contDef)
  ...
  def LoadContainerIntoCache(key: String) {
    try {
      val obj = MetadataAPIDAOUtils.GetObject(key.toLowerCase, MetadataAPIImpl.containerStore)
      ...
      MetadataAPIDAOUtils.DownloadJarFromDB(contDef)
      MetadataAPIUtils.AddObjectToCache(contDef, MdMgr.GetMdMgr)
    } catch {
      case e: Exception => {
        e.printStackTrace()
  ...
  def LoadAllContainersIntoCache {
    try {
      val contKeys = MetadataAPIUtils.GetAllKeys("ContainerDef")
      ...
      contKeys.foreach(key => {
        ...
        MetadataAPIUtils.AddObjectToCache(contDef.asInstanceOf[ContainerDef], MdMgr.GetMdMgr)
      })
    } catch {
      case e: Exception => {
        e.printStackTrace()
