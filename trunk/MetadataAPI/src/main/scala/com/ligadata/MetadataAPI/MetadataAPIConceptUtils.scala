...
object MetadataAPIConceptUtils {
  ...
  def AddConcept(attributeDef: BaseAttributeDef): String = {
    ...
    try {
      MetadataAPIDAOUtils.SaveObject(attributeDef, MdMgr.GetMdMgr)
  ...
  def RemoveConcept(concept: AttributeDef): String = {
    ...
    try {
      MetadataAPIDAOUtils.DeleteObject(concept)
    ...
    try {
      ...
      c match {
        ...
        case Some(cs) =>
          ...
          conceptArray.foreach(concept => { MetadataAPIDAOUtils.DeleteObject(concept) })
          var apiResult = new ApiResult(ErrorCodeConstants.Success, "RemoveConcept", null, ErrorCodeConstants.Remove_Concept_Successful + ":" + key)
          apiResult.toString()
      }
    } catch {
    ...
    try {
      if (format != "JSON") {
        ...
        conceptList.foreach(concept => {
          ...
          MetadataAPIDAOUtils.SaveObject(concept, MdMgr.GetMdMgr)
        })
  ...
  def LoadAttributeIntoCache(key: String) {
    try {
      val obj = MetadataAPIDAOUtils.GetObject(key.toLowerCase, MetadataAPIImpl.conceptStore)
      val cont = serializer.DeserializeObjectFromByteArray(obj.Value.toArray[Byte])
      MetadataAPIUtils.AddObjectToCache(cont.asInstanceOf[AttributeDef], MdMgr.GetMdMgr)
    } catch {
      case e: Exception => {
        e.printStackTrace()
      }
  ...
  def LoadAllConceptsIntoCache {
    try {
      val conceptKeys = MetadataAPIUtils.GetAllKeys("Concept")
      ...
      conceptKeys.foreach(key => {
        ...
        MetadataAPIUtils.AddObjectToCache(concept.asInstanceOf[AttributeDef], MdMgr.GetMdMgr)
      })
    } catch {
      case e: Exception => {
        e.printStackTrace()
