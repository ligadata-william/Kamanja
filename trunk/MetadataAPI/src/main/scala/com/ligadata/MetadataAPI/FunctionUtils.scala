...
object FunctionUtils {
  ...
  private def AddFunction(functionDef: FunctionDef): String = {
    ...
    try {
      ...
      DAOUtils.SaveObject(functionDef, MdMgr.GetMdMgr)
  ...
  def RemoveFunction(nameSpace: String, functionName: String, version: Long): String = {
    ...
    try {
      DAOUtils.DeleteObject(key, MetadataAPIImpl.functionStore)
      var apiResult = new ApiResult(ErrorCodeConstants.Success, "RemoveFunction", null, ErrorCodeConstants.Remove_Function_Successfully + ":" + key)
      apiResult.toString()
    } catch {
      case e: Exception => {
    ...
    try {
      if (format != "JSON") {
        ...
        funcList.foreach(func => {
          DAOUtils.SaveObject(func, MdMgr.GetMdMgr)
        })
  ...
  def LoadFunctionIntoCache(key: String) {
    try {
      ...
      Utils.AddObjectToCache(cont.asInstanceOf[FunctionDef], MdMgr.GetMdMgr)
    } catch {
      case e: Exception => {
        e.printStackTrace()
      }
