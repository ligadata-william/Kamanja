...
object MetadataAPITypeUtils {
  ...
  def AddType(typeText: String, format: String): String = {
    try {
      ...
      MetadataAPIDAOUtils.SaveObject(typ, MdMgr.GetMdMgr)
  ...
  def AddType(typeDef: BaseTypeDef): String = {
    try {
      ...
      MetadataAPIDAOUtils.SaveObject(typeDef, MdMgr.GetMdMgr)
    ...
    try {
      ...
      typ match {
        case None =>
          ...
          apiResult.toString()
        case Some(ts) =>
          MetadataAPIDAOUtils.DeleteObject(ts.asInstanceOf[BaseElemDef])
  ...
  def LoadTypeIntoCache(key: String) {
    try {
      ...
      val obj = MetadataAPIDAOUtils.GetObject(key.toLowerCase, MetadataAPIImpl.typeStore)
      ...
      if (typ != null) {
        ...
        MetadataAPIUtils.AddObjectToCache(typ, MdMgr.GetMdMgr)
      }
    } catch {
      case e: Exception => {
  ...
  def LoadAllTypesIntoCache {
    try {
      val typeKeys = MetadataAPIUtils.GetAllKeys("TypeDef")
