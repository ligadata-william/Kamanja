...
object MetadataAPIUtils {
  ...
  def NotifyEngine(objList: Array[BaseElemDef], operations: Array[String]) {
    try {
      ...
      if (notifyEngine != "YES") {
        ...
        MetadataAPIDAOUtils.PutTranId(objList(0).tranId)
        return
      }
  ...
  def IsDownloadNeeded(jar: String, obj: BaseElemDef): Boolean = {
    try {
      ...
      if (f.exists()) {
        val key = jar
        val mObj = MetadataAPIDAOUtils.GetObject(key, MetadataAPIImpl.jarStore)
  ...
  def UploadJar(jarPath: String): String = {
    try {
      ...
      if (!iFile.exists) {
        ...
        MetadataAPIDAOUtils.UploadJarToDB(jarPath)
    ...
    try {
      ...
      msgType match {
        case "MessageDef" | "ContainerDef" => {
          ...
          var typeDef = MetadataAPITypeUtils.GetType(msgDef.nameSpace, typeName, msgDef.ver.toString, "JSON")
          if (typeDef != None) {
            types = types :+ typeDef.get
          }
          // ArrayBufferOf<TypeName>
          typeName = "arraybufferof" + msgDef.name
          typeDef = MetadataAPITypeUtils.GetType(msgDef.nameSpace, typeName, msgDef.ver.toString, "JSON")
          if (typeDef != None) {
            types = types :+ typeDef.get
          }
  ...
  def GetAllKeys(objectType: String): Array[String] = {
    try {
      ...
      MetadataAPIImpl.metadataStore.getAllKeys({ (key: Key) => {
          val strKey = MetadataAPIDAOUtils.KeyAsStr(key)
          val i = strKey.indexOf(".")
          val objType = strKey.substring(0, i)
          val objectName = strKey.substring(i + 1)
          objectType match {
            case "TypeDef" => {
              if (MetadataAPITypeUtils.IsTypeObject(objType)) {
                keys.add(objectName)
              }
            }
            case "FunctionDef" => {
        ...
        zkMessage.ObjectType match {
          case "ModelDef" => {
            zkMessage.Operation match {
              case "Add" => {
                MetadataAPIModelUtils.LoadModelIntoCache(key)
              }
          ...
          case "MessageDef" => {
            zkMessage.Operation match {
              case "Add" => {
                MetadataAPIMessageUtils.LoadMessageIntoCache(key)
              }
              case "Remove" => {
                try {
                  MetadataAPIMessageUtils.RemoveMessage(zkMessage.NameSpace, zkMessage.Name, zkMessage.Version.toInt,false)
                } catch {
                  case e: ObjectNolongerExistsException => {
                    logger.error("The object " + key + " nolonger exists in metadata : It may have been removed already")
                  }
          ...
          case "ContainerDef" => {
            zkMessage.Operation match {
              case "Add" => {
                MetadataAPIContainerUtils.LoadContainerIntoCache(key)
              }
              case "Remove" => {
                try {
                  MetadataAPIContainerUtils.RemoveContainer(zkMessage.NameSpace, zkMessage.Name, zkMessage.Version.toInt,false)
                } catch {
                  case e: ObjectNolongerExistsException => {
                    logger.error("The object " + key + " nolonger exists in metadata : It may have been removed already")
                  }
          ...
          case "FunctionDef" => {
            zkMessage.Operation match {
              case "Add" => {
                MetadataAPIFunctionUtils.LoadFunctionIntoCache(key)
              }
              case "Remove" | "Activate" | "Deactivate" => {
                try {
          ...
          case "AttributeDef" => {
            zkMessage.Operation match {
              case "Add" => {
                MetadataAPIConceptUtils.LoadAttributeIntoCache(key)
              }
              case "Remove" | "Activate" | "Deactivate" => {
                try {
          ...
          case "JarDef" => {
            zkMessage.Operation match {
              case "Add" => {
                MetadataAPIDAOUtils.DownloadJarFromDB(MdMgr.GetMdMgr.MakeJarDef(zkMessage.NameSpace, zkMessage.Name, zkMessage.Version))
              }
              case _ => {
                logger.error("Unknown Operation " + zkMessage.Operation + " in zookeeper notification, notification is not processed ..")
              }
          ...
          case "ScalarTypeDef" | "ArrayTypeDef" | "ArrayBufTypeDef" | "ListTypeDef" | "SetTypeDef" | "TreeSetTypeDef" | "QueueTypeDef" | "MapTypeDef" | "ImmutableMapTypeDef" | "HashMapTypeDef" | "TupleTypeDef" | "StructTypeDef" | "SortedSetTypeDef" => {
            zkMessage.Operation match {
              case "Add" => {
                MetadataAPITypeUtils.LoadTypeIntoCache(key)
              }
              case "Remove" | "Activate" | "Deactivate" => {
                try {
