...
object MetadataAPIModelUtils {
    ...
    try {
      ...
      o match {
        ...
        case Some(m) =>
          ...
          MetadataAPIDAOUtils.DeactivateObject(m.asInstanceOf[ModelDef])
          ...
          MetadataAPIUtils.NotifyEngine(objectsUpdated, operations)
  ...
  def ActivateModel(nameSpace: String, name: String, version: Int): String = {
    ...
    try {
      ...
      o match {
        ...
        case Some(m) =>
          logger.debug("model found => " + m.asInstanceOf[ModelDef].FullNameWithVer)
          MetadataAPIDAOUtils.ActivateObject(m.asInstanceOf[ModelDef])
          var objectsUpdated = new Array[BaseElemDef](0)
          objectsUpdated = objectsUpdated :+ m.asInstanceOf[ModelDef]
  ...
  def RemoveModel(nameSpace: String, name: String, version: Int): String = {
    ...
    try {
      ...
      o match {
        ...
        case Some(m) =>
          logger.debug("model found => " + m.asInstanceOf[ModelDef].FullNameWithVer)
          MetadataAPIDAOUtils.DeleteObject(m.asInstanceOf[ModelDef])
          var objectsUpdated = new Array[BaseElemDef](0)
          objectsUpdated = objectsUpdated :+ m.asInstanceOf[ModelDef]
  ...
  def AddModel(model: ModelDef): String = {
    ...
    try {
      MetadataAPIDAOUtils.SaveObject(model, MdMgr.GetMdMgr)
  ...
  //    compProxy.setLoggerLevel(Level.TRACE)
      ...
      if (latestVersion != None) {
        isValid = MetadataAPIUtils.IsValidVersion(latestVersion.get, modDef)
      }
      if (isValid) {
        // save the jar file first
        MetadataAPIDAOUtils.UploadJarsToDB(modDef)
  ...
  def GetModelDefFromDB(nameSpace: String, objectName: String, formatType: String, version: String): String = {
    ...
    try {
      var obj = MetadataAPIDAOUtils.GetObject(key.toLowerCase, MetadataAPIImpl.modelStore)
      var apiResult = new ApiResult(ErrorCodeConstants.Success, "GetModelDefFromCache", MetadataAPIDAOUtils.ValueAsStr(obj.Value), ErrorCodeConstants.Get_Model_From_DB_Successful + ":" + key)
      apiResult.toString()
    } catch {
      case e: Exception => {
  ...
  def LoadModelIntoCache(key: String) {
    try {
      ...
      val obj = MetadataAPIDAOUtils.GetObject(key.toLowerCase, MetadataAPIImpl.modelStore)
      ...
      MetadataAPIDAOUtils.DownloadJarFromDB(modDef)
      logger.debug("Add the object " + key + " to the cache ")
      MetadataAPIUtils.AddObjectToCache(modDef, MdMgr.GetMdMgr)
    } catch {
      case e: Exception => {
        e.printStackTrace()
      }
  ...
  def LoadAllModelsIntoCache {
    try {
      val modKeys = MetadataAPIUtils.GetAllKeys("ModelDef")
      if (modKeys.length == 0) {
      ...
      modKeys.foreach(key => {
        ...
        MetadataAPIUtils.AddObjectToCache(modDef.asInstanceOf[ModelDef], MdMgr.GetMdMgr)
      })
    } catch {
      case e: Exception => {
        e.printStackTrace()
