...
object MetadataAPIImpl extends MetadataAPI {
  ...
  private def createAuthObj(classLoader : MetadataLoader): Unit = {
    ...
    MetadataAPIUtils.loadJar(classLoader,implJarName)
  ...
  def OpenDbStore(storeType: String) {
    try {
      ...
      metadataStore = MetadataAPIDAOUtils.GetDataStoreHandle(storeType, "metadata_store", "metadata_objects")
      configStore = MetadataAPIDAOUtils.GetDataStoreHandle(storeType, "config_store", "config_objects")
      jarStore = MetadataAPIDAOUtils.GetDataStoreHandle(storeType, "metadata_jars", "jar_store")
      transStore = MetadataAPIDAOUtils.GetDataStoreHandle(storeType, "metadata_trans", "transaction_id")
  ...
  def UpdateMessage(messageText: String, format: String): String = {
    MetadataAPIMessageUtils.UpdateMessage(messageText,format)
  }
  ...
  def UpdateContainer(messageText: String, format: String): String = {
    MetadataAPIContainerUtils.UpdateContainer(messageText,format)
  }
  ...
  def RemoveMessage(messageName: String, version: Int): String = {
    MetadataAPIMessageUtils.RemoveMessage(sysNS, messageName, version)
  }
  ...
  def RemoveMessage(nameSpace:String, messageName: String, version: Int): String = {
    MetadataAPIMessageUtils.RemoveMessage(nameSpace, messageName, version)
  }

  // Remove container with Container Name and Version Number
  def RemoveContainer(containerName: String, version: Int): String = {
    MetadataAPIContainerUtils.RemoveContainer(sysNS, containerName, version)
  }
  ...
  def RemoveContainer(nameSpace:String, containerName: String, version: Int): String = {
    MetadataAPIContainerUtils.RemoveContainer(nameSpace, containerName, version)
  }

  // Add Model (format XML)
  def AddModel(pmmlText: String): String = {
    MetadataAPIModelUtils.AddModel(pmmlText)
  }

  def UpdateModel(pmmlText: String): String = {
    MetadataAPIModelUtils.UpdateModel(pmmlText)
  }

  // Remove model with Model Name and Version Number
  def RemoveModel(modelName: String, version: Int): String = {
    MetadataAPIModelUtils.RemoveModel(sysNS, modelName, version)
  }

  // Remove model with Model Name and Version Number
  def RemoveModel(nameSpace: String,modelName: String, version: Int): String = {
    MetadataAPIModelUtils.RemoveModel(nameSpace, modelName, version)
  }

  // Activate model with Model Name and Version Number
  def ActivateModel(nameSpace: String,modelName: String, version: Int): String = {
    MetadataAPIModelUtils.ActivateModel(nameSpace, modelName, version)
  }

  // Deactivate model with Model Name and Version Number
  def DeactivateModel(nameSpace: String,modelName: String, version: Int): String = {
    MetadataAPIModelUtils.DeactivateModel(nameSpace, modelName, version)
  }

  // All available models(format JSON or XML) as a String
  def GetAllModelDefs(formatType: String): String = {
    MetadataAPIModelUtils.GetAllModelDefs(formatType);
  }

  def GetAllModelsFromCache(active: Boolean): Array[String] = {
    MetadataAPIModelUtils.GetAllModelsFromCache(active);
  }
  ...
  def GetModelDef(objectName: String, formatType: String): String = {
    MetadataAPIModelUtils.GetModelDef(sysNS, objectName, formatType)
  }
  ...
  def GetMessageDef(objectName: String, formatType: String): String = {
    ...
    MetadataAPIMessageUtils.GetMessageDefFromCache(nameSpace, objectName, formatType, "-1")
  }
  ...
  def GetMessageDef(objectName: String, version: String, formatType: String): String = {
    val nameSpace = MdMgr.sysNS
    MetadataAPIMessageUtils.GetMessageDef(nameSpace, objectName, formatType, version)
  }
  ...
  def GetContainerDef(objectName: String, formatType: String): String = {
    val nameSpace = MdMgr.sysNS
    MetadataAPIContainerUtils.GetContainerDefFromCache(nameSpace, objectName, formatType, "-1")
  }
  ...
  def GetContainerDef(objectName: String, version: String, formatType: String): String = {
    val nameSpace = MdMgr.sysNS
    MetadataAPIContainerUtils.GetContainerDef(nameSpace, objectName, formatType, version)
  }
  ...
  def GetAllFunctionDefs(formatType: String): (Int,String) = {
    MetadataAPIFunctionUtils.GetAllFunctionDefs(formatType)
  }

  def GetAllFunctionsFromCache(active: Boolean): Array[String] = {
    MetadataAPIFunctionUtils.GetAllFunctionsFromCache(active)
  }

  def GetFunctionDef(nameSpace: String, objectName: String, formatType: String): String = {
    MetadataAPIFunctionUtils.GetFunctionDef(nameSpace,objectName,formatType)
  }
  ...
  def GetAllConcepts(formatType: String): String = {
    MetadataAPIConceptUtils.GetAllConcepts(formatType)
  }

  def GetAllConceptsFromCache(active: Boolean): Array[String] = {
    MetadataAPIConceptUtils.GetAllConceptsFromCache(active)
  }
  ...
  def GetConcept(nameSpace:String, objectName: String, version: String, formatType: String): String = {
    MetadataAPIConceptUtils.GetConcept(nameSpace,objectName,version,formatType)
  }

  // A single concept as a string using name and version as the key
  def GetConcept(objectName: String, version: String, formatType: String): String = {
    MetadataAPIConceptUtils.GetConcept(MdMgr.sysNS,objectName,version,formatType)
  }
  ...
  def GetConcept(objectName: String, formatType: String): String = {
    MetadataAPIConceptUtils.GetConcept(objectName,formatType)
  }

  // All available derived concepts(format JSON or XML) as a String
  def GetAllDerivedConcepts(formatType: String): String = {
    MetadataAPIConceptUtils.GetAllDerivedConcepts(formatType)
  }
  ...
  def GetDerivedConcept(objectName: String, formatType: String): String = {
    MetadataAPIConceptUtils.GetDerivedConcept(objectName,formatType)
  }
  ...
  def AddNode(nodeId:String,nodePort:Int,nodeIpAddr:String,
    ...
    MetadataAPIClusterCfgUtils.AddNode(nodeId,nodePort,nodeIpAddr,jarPaths,scala_home,
  ...
  def RemoveNode(nodeId:String) : String = {
    MetadataAPIClusterCfgUtils.RemoveNode(nodeId)
  }
  ...
  def AddAdapter(name:String,typeString:String,dataFormat: String,className: String, 
    ...
    MetadataAPIClusterCfgUtils.AddAdapter(name,typeString,dataFormat,className,jarName,
  ...
  def RemoveAdapter(name:String) : String = {
    MetadataAPIClusterCfgUtils.RemoveAdapter(name)
  }


  def AddCluster(clusterId:String,description:String,privileges:String) : String = {
    MetadataAPIClusterCfgUtils.AddCluster(clusterId,description,privileges)    
  }
  ...
  def RemoveCluster(clusterId:String) : String = {
    MetadataAPIClusterCfgUtils.RemoveCluster(clusterId)
  }
  ...
  def logAuditRec(userOrRole:Option[String], userPrivilege:Option[String], action:String, objectAccessed:String, success:String, transactionId:String, notes:String) = {
    ...
    if( auditObj != null ){
      ...
      aRec.actionTime = MetadataAPIUtils.getCurrentTime
      aRec.action = action
  ...
  def getAuditRec(filterParameters: Array[String]) : String = {
	...
	paramCnt match {
	  case 1 => {
	    startTime = MetadataAPIUtils.parseDateStr(filterParameters(0))
	  }
	  case 2 => {
	    startTime = MetadataAPIUtils.parseDateStr(filterParameters(0))
	    endTime = MetadataAPIUtils.parseDateStr(filterParameters(1))
	  }
	  case 3 => {
	    startTime = MetadataAPIUtils.parseDateStr(filterParameters(0))
	    endTime = MetadataAPIUtils.parseDateStr(filterParameters(1))
  ...
  def LoadAllObjectsIntoCache {
    try {
      val configAvailable = MetadataAPIClusterCfgUtils.LoadAllConfigObjectsIntoCache
      if( configAvailable ){
	...
	logger.debug("Assuming bootstrap... No config objects in persistent store")
      ...
      val maxTranId = MetadataAPIDAOUtils.GetTranId
      ...
      keyArray.foreach(key => {
        val obj = MetadataAPIDAOUtils.GetObject(key, metadataStore)
        ...
        if (mObj != null) {
          if (mObj.tranId <= maxTranId) {
            MetadataAPIUtils.AddObjectToCache(mObj, MdMgr.GetMdMgr)
            MetadataAPIDAOUtils.DownloadJarFromDB(mObj)
          } else {
            logger.debug("The transaction id of the object => " + mObj.tranId)
            MetadataAPIUtils.AddObjectToCache(mObj, MdMgr.GetMdMgr)
            MetadataAPIDAOUtils.DownloadJarFromDB(mObj)
            logger.error("Transaction is incomplete with the object " + MetadataAPIDAOUtils.KeyAsStr(key) + ",we may not have notified engine, attempt to do it now...")
      ...
      if (objectsChanged.length > 0) {
        MetadataAPIUtils.NotifyEngine(objectsChanged, operations)
      }
      startup = false
    } catch {
      case e: Exception => {
