...
object MetadataAPIImpl extends MetadataAPI {
    ...
    if (fl.exists) {
      try {
        ...
        classLoader.loadedJars += fl.getPath()
          ...
          case e: Exception => {
            val stackTrace = Utils.ThrowableTraceString(e)
            logger.error("Failed to add "+implJarName + " due to internal exception " + "\nStackTrace:"+stackTrace)
            return
          }
        }
    } else {
  ...
  def SaveObject(key: String, value: Array[Byte], store: DataStore) {
    ...
    try {
      ...
      case e: Exception => {
        val stackTrace = Utils.ThrowableTraceString(e)
        logger.error("Failed to insert/update object for : " + key + ", Reason:" + e.getCause + ", Message:" + e.getMessage+"/nStackTrace:"+stackTrace)
        store.endTx(t)
...
*/
        ...
        zkMessage.Operation match {
          ...
          case "Remove" | "Activate" | "Deactivate" => {
            try {
              ...
              case e: ObjectNolongerExistsException => { val stackTrace = Utils.ThrowableTraceString(e)
  ...
  def UpdateMdMgr(zkTransaction: ZooKeeperTransaction): Unit = {
    ...
    try {
      ...
      case e: AlreadyExistsException => { val stackTrace = Utils.ThrowableTraceString(e)
