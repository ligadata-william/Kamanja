...
object TestMessageDefImpl extends LogTrait {
  ...
  private def processContainers(msg: MessageDefImpl): Unit = {
    ...
    val (classStr1: String, msgDef1: ContainerDef) = msg.processMsgDef(json1.toString(), "JSON", MdMgr.GetMdMgr)
  ...
  private def processFixedBeneficiary(msg: MessageDefImpl): Unit = {
    ...
    val json100: String = Source.fromFile("/tmp/testing/messages/hl7_101.json").getLines.mkString
    val (classStr200: String, msgDef200: ContainerDef) = msg.processMsgDef(json100.toString(), "JSON", MdMgr.GetMdMgr, false)
