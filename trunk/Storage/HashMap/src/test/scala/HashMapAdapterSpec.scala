...
package com.ligadata.automation.unittests.hashmapadapter
...
class HashMapAdapterSpec extends FunSpec with BeforeAndAfter with BeforeAndAfterAll with GivenWhenThen {
  var res : String = null;
  ...
  describe("Unit Tests for all hashmapadapter operations") {
	...
	adapter.get(containerName,readCallBack _)
      ...
      hashmapAdapter = adapter.asInstanceOf[HashMapAdapter]

      And("Check the row count after adding a bunch")
      var cnt = hashmapAdapter.getRowCount(containerName)
      assert(cnt == 10)
	...
	adapter.del(containerName,keys)
      ...
      cnt = hashmapAdapter.getRowCount(containerName)
      assert(cnt == 0)
