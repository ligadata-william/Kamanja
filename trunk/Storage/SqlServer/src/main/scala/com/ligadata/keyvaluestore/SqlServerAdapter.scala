...
class SqlServerAdapter(val kvManagerLoader: KamanjaLoaderInfo, val datastoreConfig: String) extends DataStore {
  ...
  private def IsSchemaExists(schemaName: String): Boolean = {
	...
	return false
      ...
      case e: Exception => {
        throw CreateDMLException("Failed to verify schema existence for the schema " + schemaName + ":" + e.getMessage(),e)
      }
    } finally {
      if (rs != null) {
        rs.close
  ...
  override def put(containerName: String, key: Key, value: Value): Unit = {
    ...
    try {
      ...
      case e: Exception => {
	con.rollback()
        throw CreateDMLException("Failed to save an object in the table " + tableName + ":" + e.getMessage(),e)
      }
    } finally {
      if (cstmt != null) {
        cstmt.close
  ...
  override def put(data_list: Array[(String, Array[(Key, Value)])]): Unit = {
	...
	logger.debug("Executing bulk Insert...")
      ...
      case e: Exception => {
	con.rollback()
        throw CreateDMLException("Failed to save a batch of objects into the table :" + e.getMessage(),e)
      }
    } finally {
      if (pstmt != null) {
        pstmt.close
  ...
  override def del(containerName: String, keys: Array[Key]): Unit = {
    ...
    try {
      ...
      case e: Exception => {
	con.rollback()
        throw CreateDMLException("Failed to delete object(s) from the table " + tableName + ":" + e.getMessage(),e)
      }
    } finally {
      if (cstmt != null) {
        cstmt.close
  ...
  private def TruncateContainer(containerName: String): Unit = {
    ...
    try {
      ...
      case e: Exception => {
        throw CreateDDLException("Failed to fetch data from the table " + tableName + ":" + e.getMessage(),e)
      }
    } finally {
      if (stmt != null) {
        stmt.close
