...
"""
    ...
    override def main (args : Array[String]) {
        ...
        def nextOption(map: OptionMap, list: List[String]): OptionMap = {
            list match {
              ...
              case "--systemlib" :: value :: tail =>
                nextOption(map ++ Map('systemlib -> value), tail)
	      case _ =>
                projects += list.head
        ...
        println(s"all projects to be processed : ${projects}.toArray") 

	val options = nextOption(Map(), arglist)

        val excludedProjects : String = if (options.contains('excludeProjects)) options.apply('excludeProjects) else null
        val systemlib : String = if (options.contains('systemlib)) options.apply('systemlib) else null
        println(s"systemlib = $systemlib")
	val projectsToConsider : Array[String] = if (excludedProjects != null) {
        ...
        projectsToConsider.foreach(arg => {
            ...
            val depJarsCmd = s"bash SampleApplication/EasyInstall/sbtProjDependencies.scala --sbtDeps ${'"'}`sbt 'show $arg/fullClasspath' | grep 'List(Attributed'`${'"'} --emitCp 1"
        ...
        println(s"Dependency jars for repo found at $systemlib ")
        allDepJars.foreach(jarPath => {
	val copyjars = s"cp $jarPath $systemlib"
	Process(copyjars).!!.trim
	println("cp " + jarPath + " $systemlib")})
	}
}
