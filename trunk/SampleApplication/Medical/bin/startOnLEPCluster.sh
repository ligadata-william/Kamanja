...
#	example: startFatafatCluster.sh --MetadataAPIConfig SampleApplication/Medical/Configs/MetadataAPIConfig.properties --NodeConfigPath SampleApplication/Medical/Configs/Engine2BoxConfigV1.json 
#
...
# 1) determine which machines and installation directories are to get the build from the metadata and Fatafat config
...
echo "...extract node information for the cluster to be installed from the Metadata and optional Fatafat config supplied"
if  [ "$#" -eq 4 ]; then
...
echo "...start the Fatafat cluster "
...
while read LINE; do
    ...
    echo "...On machine $machine, starting FatafatManager with configuration $cfgFile for nodeId $id to $machine:$targetPath"
    scp "$cfgFile" "$machine:$targetPath/"
	ssh -T $machine  <<-EOF
	        cd $targetPath
	        nodeCfg=`echo $cfgFile | sed 's/.*\/\(.*\)/\1/g'`
	        java -jar ./FatafatManager-1.0 --config "./$cfgFile" &
EOF
done
exec 0<&12 12<&-
