...
class FieldColumnPairCodePrinter(ctx : PmmlContext) extends CodePrinter with com.ligadata.pmml.compiler.LogTrait {
	...
	def print(node : Option[PmmlExecNode]
		...
		val xnode : xFieldColumnPair = node match {
			case Some(node) => {
				if (node.isInstanceOf[xFieldColumnPair]) node.asInstanceOf[xFieldColumnPair] else null
			}
			case _ => null
		}
		...
		val printThis = if (xnode != null) {
			...
			if (node != null) {
				PmmlError.logError(ctx, s"For ${xnode.qName}, expecting an xFieldColumnPair... got a ${xnode.getClass.getName}... check CodePrinter dispatch map initialization")
			}
			""
		}
		printThis
	...
	private def codeGenerator(node : xFieldColumnPair
							, generator : CodePrinterDispatch
							, kind : CodeFragment.Kind
							, traversalOrder : Traversal.Order) : String = 	{
		...
		val mapVM : String = traversalOrder match {
			...
			case Traversal.PREORDER => {
				s"ctx.dDict.apply(${'"'}FieldColumnPair ${node.field}, ${node.column}${'"'})"
			}
		}
		mapVM
	}
