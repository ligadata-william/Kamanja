...
object NodePrinterHelpers extends com.ligadata.pmml.compiler.LogTrait {
	...
	def modelClassComment(ctx : PmmlContext, generator : CodePrinterDispatch) : String = {
		...
		/** Should the namespace not be unique for this model, then there **can** be name clashes for the derived fields and
		 *  other model nodes (e.g., SimpleRule) between models.  Should those models run at the same time (in the working set),
		 *  there will be odd allocations where the Derived_<fieldname> for a model 1 has the same name as model 2's Derived_<fieldname>
		 *  When these two Derived_<fieldname> classes are in the same namespace, the first one is allocated, which causes
		 *  sighing and groaning among model authors and pmml platform developers alike.
		 */
		/** FIXME: Even with this check, a race condition with another pmml compiler running could cause an issue ... */
		if (ctx.KnownNamespacesAtCompileTime.contains(modelPkg.toLowerCase)) {
			PmmlError.logError(ctx, s"Namespace $modelPkg already exists.  Classes generated by the pmml compiler in the contemplated namespace could have fqNames that are the same as a result of other compilations.")
			logger.error("Choose a unique namespace for your model that will prevent this unfortunate circumstance.")
			ctx.IncrErrorCounter
		}		
