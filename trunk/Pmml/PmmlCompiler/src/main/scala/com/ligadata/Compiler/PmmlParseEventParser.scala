...
class PmmlParseEventParser(ctx : PmmlContext)  extends DefaultHandler {
	var start = false
	var node: PmmlNode = _
	...
	override def startElement(namespaceURI: String, localName: String , qName:String , atts: Attributes ) = {
		...
		if (ctx.pmmlElementVistitorMap.contains(qName)) {
			ctx.logger.trace(s"...encountered $qName")
			...
			//if (qName == "SimpleRule") {
				//ctx.logger.trace("reached SimpleRule")
			//}
		} else {
			ctx.logger.trace(s"...element $qName is of no interest at all")
			//ctx.dispatchElementVisitor(ctx, namespaceURI, localName , qName, atts)
		}
	}
	...
	override def endElement( uri: String, localName: String, qName: String) = {
  		...
  		if (pmmlnode == null) {
  			ctx.logger.debug(s"endElement... pmmlNodeStack is empty ... qName = $qName")
  		} else {

			val buf : String = buffer.toString().trim
			val len = buf.length()
			if (buf.length() > 0) {
				ctx.logger.trace(s"the following string was collected for $qName (length = $len) : $buf")
