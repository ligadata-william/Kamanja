...
class MapValuesMapCodePrinter(ctx : PmmlContext) extends CodePrinter with com.ligadata.pmml.compiler.LogTrait {
	...
	def print(node : Option[PmmlExecNode]
		...
		val xnode : xMapValuesMap = node match {
			case Some(node) => {
				if (node.isInstanceOf[xMapValuesMap]) node.asInstanceOf[xMapValuesMap] else null
			}
			case _ => null
		}
		...
		val printThis = if (xnode != null) {
			...
			if (node != null) {
				PmmlError.logError(ctx, s"For ${xnode.qName}, expecting an xMapValuesMap... got a ${xnode.getClass.getName}... check CodePrinter dispatch map initialization")
			}
			""
		}
		printThis
	...
	private def codeGenerator(node : xMapValuesMap
							, generator : CodePrinterDispatch
							, kind : CodeFragment.Kind
							, traversalOrder : Traversal.Order) : String = 	{
		...
		val mapVM : String = traversalOrder match {
			...
			case Traversal.PREORDER => {
				s"ctx.dDict.apply(${'"'}MapValuesMap ${node.outputColumn}${'"'})"
			}
		}
		mapVM
	}
