...
object NodePrinterHelpers extends LogTrait {
	...
	def modelClassBody(ctx : PmmlContext, generator : PmmlModelGenerator) : String = {
		...
		clsBuffer.append(s"class $nmspc$classname$verNoStr($ctorGtxAndMessagesStr, val modelName:String, val modelVersion:String, val tenantId: String, val transId: Long)\n")
		if (ctx.injectLogging) {
