...
class RuleSelectionMethodCodePrinter(ctx : PmmlContext) extends CodePrinter with com.ligadata.pmml.compiler.LogTrait {
	...
	def print(node : Option[PmmlExecNode]
		...
		val xnode : xRuleSelectionMethod = node match {
			case Some(node) => {
				if (node.isInstanceOf[xRuleSelectionMethod]) node.asInstanceOf[xRuleSelectionMethod] else null
			}
			case _ => null
		}
		...
		val printThis = if (xnode != null) {
			...
			if (node != null) {
				PmmlError.logError(ctx, s"For ${xnode.qName}, expecting an xRuleSelectionMethod... got a ${xnode.getClass.getName}... check CodePrinter dispatch map initialization")
			}
			""
		}
		printThis
	...
	private def codeGenerator(node : xRuleSelectionMethod
							, generator : CodePrinterDispatch
							, kind : CodeFragment.Kind
							, traversalOrder : Traversal.Order) : String = 	{

		val selMeth : String = traversalOrder match {
			case Traversal.INORDER => { "" }
			case Traversal.POSTORDER => { "" }
			case Traversal.PREORDER => {
				s" new RuleSelectionMethod(${'"'}${node.criterion}${'"'})"
			}
		}
		selMeth
	}
