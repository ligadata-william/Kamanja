...
class FieldRefCodePrinter(ctx : PmmlContext) extends CodePrinter with com.ligadata.pmml.compiler.LogTrait {
	...
	def print(node : Option[PmmlExecNode]
		...
		val xnode : xFieldRef = node match {
			case Some(node) => {
				if (node.isInstanceOf[xFieldRef]) node.asInstanceOf[xFieldRef] else null
			}
			case _ => null
		}
	...
	private def codeGenerator(node : xFieldRef
							, generator : CodePrinterDispatch
							, kind : CodeFragment.Kind
							, traversalOrder : Traversal.Order) : String = 	{
		...
		val fldRef : String = traversalOrder match {
			...
			case Traversal.PREORDER => {
				...
				PmmlExecNode.prepareFieldReference(ctx, node.field)
			}
		}
		fldRef
	}
