...
class MapValuesMapInlineCodePrinter(ctx : PmmlContext) extends CodePrinter with com.ligadata.pmml.compiler.LogTrait {
	...
	def print(node : Option[PmmlExecNode]
		...
		val printThis = if (xnode != null) {
			...
			if (node != null) {
				PmmlError.logError(ctx, s"For ${xnode.qName}, expecting an xMapValuesMapInline... got a ${xnode.getClass.getName}... check CodePrinter dispatch map initialization")
			}
			""
		}
		printThis
