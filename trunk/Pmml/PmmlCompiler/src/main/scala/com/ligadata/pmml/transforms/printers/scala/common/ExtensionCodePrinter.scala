...
class ExtensionCodePrinter(ctx : PmmlContext) {
	...
	def print(node : Option[PmmlExecNode]
		...
		val printThis = if (xnode != null) {
			...
			if (node != null) {
				PmmlError.logError(ctx, s"For ${node.qName}, expecting an xExtension... got a ${node.getClass.getName}... check CodePrinter dispatch map initialization")
			}
			""
		}
		printThis
