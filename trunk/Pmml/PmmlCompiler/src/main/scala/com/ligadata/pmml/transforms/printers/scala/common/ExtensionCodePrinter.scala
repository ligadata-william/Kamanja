...
class ExtensionCodePrinter(ctx : PmmlContext) extends CodePrinter with com.ligadata.pmml.compiler.LogTrait {
	...
	def print(node : Option[PmmlExecNode]
		...
		val xnode : xExtension = node match {
			case Some(node) => {
				if (node.isInstanceOf[xExtension]) node.asInstanceOf[xExtension] else null
			}
			case _ => null
		}
		...
		val printThis = if (xnode != null) {
			...
			if (node != null) {
				PmmlError.logError(ctx, s"For ${xnode.qName}, expecting an xExtension... got a ${xnode.getClass.getName}... check CodePrinter dispatch map initialization")
			}
			""
		}
		printThis
	...
	private def codeGenerator(node : xExtension
							, generator : CodePrinterDispatch
							, kind : CodeFragment.Kind
							, traversalOrder : Traversal.Order) : String = 	{

		val ext : String = traversalOrder match {
			case Traversal.INORDER => { "" }
			case Traversal.POSTORDER => { "" }
			case Traversal.PREORDER => {

				s"Extension (extender = ${node.extender}, name = ${node.name}, value = ${node.value})"
			}
		}
		ext
	}
